...
📂 Acessar arquivos desta atividade: minha_playlist.txt
...
Endpoints desenvolvidos:
GET /personagens/todos — Lê o arquivo personagens_db.csv.txt, faz o split(";") e instancia as classes filhas corretas.
...
📂 Acessar arquivos desta atividade: personagens_db.csv.txt

🏆 3. O Portal do Hackathon

O que faz: Atua como o painel de controle de um Hackathon. Ao ser acionado pela web, o servidor processa em lote um arquivo de inscrições brutas (inscricoes_brutas.txt), valida as regras de idade (14 a 21 anos), gera arquivos separados de aprovados e pendências, e retorna um relatório estatístico em tempo real.
Endpoints desenvolvidos:
POST /hackathon/processar — Valida as inscrições, gera os arquivos locais de saída (aprovados_hackathon.txt e pendencias_inscricoes.txt) e retorna um JSON com o resumo do processamento.
...
📂 Acessar arquivos desta atividade: inscricoes_brutas.txt | aprovados_hackathon.txt | pendencias_inscricoes.txt
...
📄 Ver documentação Swagger (swagger.yaml)
Para visualizar a documentação de forma interativa, basta copiar o conteúdo do arquivo e colar no Swagger Editor.

🛠️ Tecnologias Utilizadas

TecnologiaFunção no ProjetoJavaLinguagem de programação principal do ecossistema.Spring BootFramework para criação da API REST e gerenciamento do servidor web local.OpenAPI / SwaggerDocumentação e testes interativos dos endpoints da API.JSONFormato de comunicação e transferência de dados entre a API e o cliente.TXT / CSVUtilizados como persistência local de dados (simulando bancos de dados em arquivos).
